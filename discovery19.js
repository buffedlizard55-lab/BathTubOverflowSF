...
exportButton.addEventListener('click', () => {
  ...
  const a = document.createElement('a'); a.href = url; a.download = 'sunset-discovery-wave19.csv'; a.click(); setTimeout(() => URL.revokeObjectURL(url), 1000);
});
try {
  const response = await fetch('./data/wave19.json');
  ...
  queue.innerHTML = `<p class="empty">${e(error.message)} Reload the page or open <a href="./data/wave19.json">the source dataset</a>.</p>`;
}
