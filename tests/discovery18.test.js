...
const wave = JSON.parse(readFileSync(new URL('../data/wave18.json', import.meta.url)));
...
test('wave 18 pass 1: every retained registry field has a source transcription', () => {
...
test('wave 18 pass 2: no known ID, name, core or registry-number collision', () => {
  ...
  // The folded business must be the corpus record, never a parallel entry.
  assert.equal(wave.dedupeFolds.length, 1);
  assert.equal(wave.dedupeFolds[0].storedAs, 'w10-repipe-specialists-san-francisco-bay-area');
  assert.ok(ids.has(wave.dedupeFolds[0].storedAs));
});

test('wave 18 pass 3: fail closed, leave prior corpus and master untouched', () => {
  ...
  assert.equal(byStatus('active').length, 2);
  assert.equal(byStatus('suspended').length, 2);
  assert.equal(byStatus('inactive').length, 2);
  assert.equal(byStatus('expired').length, 6);
  assert.deepEqual(byStatus('active').map(b => b.licenseCheck.number).sort(), ['711115', '730800']);
  assert.deepEqual(byStatus('suspended').map(b => b.licenseCheck.number).sort(), ['750209', '819519']);
  assert.deepEqual(byStatus('inactive').map(b => b.licenseCheck.number).sort(), ['719222', '830655']);
  const classes = Object.fromEntries(checks.map(b => [b.licenseCheck.number, b.licenseCheck.classes.join('+')]));
  assert.equal(classes['750209'], 'C35');
  assert.equal(classes['830655'], 'C35');
  assert.equal(classes['819519'], 'B+C36');
  assert.equal(classes['711115'], 'C33');
  for (const n of ['721644', '730800', '719222', '847215', '845353', '791127', '685718', '749150'])
    assert.equal(classes[n], 'B', n);
  assert.equal(wave.reviews.length, 6);
  assert.equal(wave.reviews.filter(r => r.kind === 'review').length, 5);
  assert.equal(wave.reviews.filter(r => r.kind === 'business-description').length, 1);
  // Truncation honesty: cut excerpts must say so instead of being completed.
  for (const r of wave.reviews.filter(r => r.truncatedStart || r.truncatedEnd))
    assert.ok(r.note.length > 0, r.id);
});

test('wave 18 privacy: no excluded private-context marker in any field', () => {
  assertPrivacySafe(JSON.stringify(wave), 'wave18.json');
});
